...
shelters <- st_read("../data/shelters.json") # use tab to find files


```


```{r checkTheFiles, eval=FALSE, echo=TRUE}

# head() says the geometry type (what format), what CRS is specified (projected or geodetic) etc.
...
*ANSWER)* forests has 16 features/variable; parks has 11; playgrounds has projected CRS with 'ETRS89 / UTM zone 32N' and shelters has 7
*ANSWER)* forests' features are for multiPOLYGONS; parks has multiPOLYGONS; playgrounds has points and shelters has points


2. What is the CRS value in these objects?
*ANSWER)* forests uses projected CRS w. the values 'ETRS89 / UTM zone 32N'; parks has projected CRS with 'ETRS89 / UTM zone 32N'; playgrounds has not specified and shelters has projected CRS with 'ETRS89 / UTM zone 32N'
...
print("number of parks greater than 5ha")
nrow(parksOver5ha)

# Limit to navn and areal_m2 variables
parksOver5ha_new <- select(parksOver5ha, c("navn", "areal_m2"))

# Use head() to look at the first few records
head(parksOver5ha_new)


```

*ANSWER)* the general metadata is still included, as well as the geometry column defining what the format is (multi-polygons) where the polygons are. Geometry column is included by default
...
# This will not work - it returns logical values only
...
or (if unclass() doesn't work) you need to convert `val`'s class to `units`, for example:
...
areaForest <- st_area(forests)
hist(areaForest) # visualise the data spread of areas

# back to working with the forests object
forests <- forests %>% filter(as.numeric(unclass(areal_m2)) > 200000)
...
# st_geometry() is the same as just choosing the geometry column from the object
...
forests_onlyGeometry <- st_geometry(forests)
plot(forests_onlyGeometry)
plot(forests_onlyGeometry, col = "darkgreen");plot(st_geometry(parks), col = "pink", add=TRUE) # cool ability to plot multiple layers


# issue adding a layer for playgrounds, because the playgrounds data has not specified the CRS. Thus it doesn't add these datapoint to the plot of the others cause it doesn't know how their projection relates to the projection of playgrounds
...
pacman::p_load(raster, terra)
elevation <- raster("../data/DNK_msk_alt.grd")
aarhus <- terra::rast("../data/Aarhus_1m.TIF") # use terra for multi-layer/multi-band objects
...
*ANSWER)* the elevation data has 1 dimension; Representing denmark's elevation

2. What is the resolution of `aarhus` orthophoto? How many layers does it contain and what do they represent?

*ANSWER)* the aarhus data has 5 dimensions and a resolution of 1, 1, 1. Four of the layers represent part of aarhus
...
forests
st_crs(forests) # projected

playgrounds # latlong, given the points' coordinates
st_crs(playgrounds) <- 4326
playgrounds

# transform playgrouns to go from 3D to 2D to be compatible with forests
playgrounds_2D_32 <- st_transform(playgrounds, crs = 25832)
st_crs(playgrounds_2D_32)
playgrounds_2D_32 #geometry fits 2D


## PLOTTING
plot(forests$geometry, col = "darkgreen");plot(playgrounds_2D_32$geometry, add=TRUE, col = "pink");plot(shelters$geometry, add=TRUE, col = "brown");plot(parks$geometry, add=TRUE, col = "lightgreen")
...
the_crs <- sf::st_crs(aarhus, asText = TRUE)



elevation_32 <- projectRaster(elevation, crs = 25832) # different way of assigning the new CRS
...
plot(aarhus)
plotRGB(aarhus, alpha = 100);   #confusion: why this function
plot(playgrounds_2D_32, add=TRUE, col = "green", lwd = 3)
...
I have prepared a `mystery.zip` folder for you at this link: [Download the mysterious files](https://bit.ly/mysteriousfiles) They are all in Denmark, but they have different CRSs and file formats. Your job is to read them in R and create two plots: 1) one where you make all layers line up and 2) where you select and filter layers to create as pretty map as 
...
There are a lot of online resources for spatial data, such as OSM, AirBnB, GoogleMaps, DivaGIS, and GADM (global administrative database). Load some of this spatial data - vector and raster  -  using the `geodata` package. GADM has data for all current countries and is well-integrated with R. You can load the data directly into R with the function `geodata::gadm()` and start processing. The available data are:
