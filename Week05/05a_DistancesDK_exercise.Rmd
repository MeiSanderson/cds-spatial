...
points <- data.frame(names =c("Aarhus University", "Sydney Opera"), 
                        longitude = c(my_points[1], my_points[2]),
                        latitude = c(my_points[3], my_points[4]))

# create geographic simple feature # 3D
points_4326 <- points %>% 
  st_as_sf(coords = c("longitude", "latitude"), crs = 4326)

# create a planimetric simple feature # 2D
points_32632 <- points %>% 
  st_as_sf(coords = c("longitude", "latitude"), crs = 4326) %>% 
  st_transform(32632)

# OR
#points_32632 <- points %>% points_4326 %>% st_transform(32632)
