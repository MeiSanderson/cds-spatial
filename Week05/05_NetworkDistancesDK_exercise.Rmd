...
my_token <- "pk.eyJ1IjoibWVpLXNhbmRlcnNvbiIsImEiOiJjbW1ieGFsbTUwMGVtMnBxd2F5Mzc5aTA5In0.upzLj_8JPj698LPKsVPEYw" ## IF sharing; like on GitHub -> DO THE BELOW (save and loaded as .txt file in _gitignore_), so that the token is shared with others
...
mapbox_map <- leaflet() %>%
  addMapboxTiles(style_id = "streets-v11",
                 username = "mapbox") # agreed to use Mapbox layers
...
nobel # often not that accurate; it just gives one of the proposed coordinates; be as specific as possible. like use Christian instead of Chr.

nobel_better <- mb_geocode("Jens Christian Skous Vej 5, Aarhus, 8000, Denmark")
nobel_better


myNobel <- c(56.1725600263473, 10.205569386508923)
...
# with lng and lat found by us
mapbox_map %>% 
  setView(lng = myNobel[2],
          lat = myNobel[1], 
          zoom = 14)
...
nobel_iso_drive <- mb_isochrone(nobel_better,
                                 profile = "cycling",
                                 time = c(4, 8, 12)) # minutes
...
route <- mb_directions(origin = nobel,
                       ...
                       profile = "cycling") # again poor at finding the right adress

route


myCoor <- c(56.08887140134115, 10.22352315420018)
myRoute <- mb_directions(origin = nobel,
                       destination = "Moesgaard Museum, Aarhus, Denmark",
                       profile = "cycling") # NOT SURE HOW TO ADD myCoor

mapbox_map %>%
  addPolylines(data = myRoute, 
               popup = paste0(
                 "Distance (km): ",
...
#isos <- mb_isochrone(
#  location = mb_geocode("Palle Juul-Jensens Blvd. 161, 8200 Aarhus"),
#  profile = "cycling",
#  time = c(4, 8, 12))
...
# the above is a bunch of polygons, thus we make it to a raster
...
isos_32 <- st_transform(isos, crs = 25832)

isos <- vect("../data/isos4326.shp")
isos_32 <- terra::project(isos, "EPSG:25832")

# Create a RasterLayer (SpatRaster in terra) with the specified resolution of 100m
# template <- raster(isos_32, resolution = 100) # simple object
template <- terra::rast(isos_32, resolution = 100) # terra

# Use the rasterize function from terra, specifying the field and aggregation function
iso_surface <- terra::rasterize(isos_32, template, field = "time", fun = min, background=NA) # if raster() before, then use fasterize library
...
my_opencage_key = "8298a49c3afb4fe49a2771cc1e2e7c4d"
Sys.setenv(OPENCAGE_KEY = my_opencage_key)

# OR
# set the key interactively if it is missing.
oc_config() # input key/token in console

# OR, as previously mentioned, include it in a -txt file in an ignore folder
...
h <- oc_forward_df(placename = hospitals, countrycode = "DK", limit = 1, no_annotations = TRUE) # limit = 1; it chooses the first address to come up based on the string descriptions of the addresses from above
...
# h <- readRDS("../data/Sciencedata/hospitals.rds") # better using direct adresses
...
mapbox_map %>% addPolygons(data = driving_isos, color = "yellow") %>% addPolygons(data = cycling_isos, color = "orange") %>% addPolygons(data = walking_isos, color = "red")
...
## ASSIGNMENT WORK
```{r kommune and hospitals, eval = FALSE}

# st_union = to work with a big object/polygon; important to use
# st_intersection = to get intersection between areas-not-25%-away-from-hospital and municipalities


# FUTURE -> incl. ambulance "stations"; incl. visualisations of the % of no-car in these areas
# mapbox API doesn't include traffic



# IDEA
# hospitals -> isochrome -> st_union -> ***
