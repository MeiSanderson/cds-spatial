...
pacman::p_load(knitr,
               ...
               latticeExtra,
               RColorBrewer,
               rgl,  # macs might need another prerequisite
               landsat,
               itcSegment,
               RStoolbox,
               rasterVis,
...
library(landsat)
library(rasterVis)
#rglwidget() # meant to aid rmd conversion to html
plot3D(dem, rev=T, zfac=1) # good for visualisation; little for any analysis
...
# if comparing histograms, then equalise histograms between observation times (e.g., if one is from june and another from november)
...
## better
plotRGB(myRGB, stretch = "hist") # strech linearly 'lin' or histogram 'hist'
plotRGB(myCIR,  stretch = "hist") # false colour image
```
...
# if used for analyse mask away the clouds and cloud shadows
```
...
cldmsk    <- cloudMask(myCIR, blue = 4, tir = 1) # could mask function
ggR(cldmsk, 2, geom_raster = TRUE) # geom_raster plots faster, 2 indicates the MASK

# but the colour for the cloud shadows seem too much like some of the meaningful values on the map, thus we will just subtrct the clouds and keep the shadows
```
...
ggRGB(myCIR, stretch = "lin") +
  ...
  scale_fill_manual(values = c("yellow"), na.value = NA)  # everything below threshold is said to be a cloud; 5 points buffer
```

# Task 7: Create new information from satellite imagery: NDVI

Let us calculate the _Normalized Difference Vegetation Index_ (NDVI) and see where the vegetation grows most in our Landsat image:
...
# uncomment and run in 3D -> the plot seems better
...
# the lighter the colour, the healtier the vegetation in november; likely the watered plants in people's forests
```

# Task 9: Unsupervised Classification with k-means (clustering)
...
names(myNewBrick) <- c("NIR", "RED", "GREEN", "BLUE", "NDVI")
splom(myNewBrick)
plot(myNewBrick)
...
clusters <- rast(myNewBrick)   ## create an empty raster with same extent/shape than ICE
...
forest <- terra::rast("../data/Sciencedata/imgData.tif")

plot(forest)

# Use the itcIMG() function to detect and grow the individual crowns
?itcIMG()
se <- itcIMG(forest, epsg=32632) # try this out: https://brightspace.au.dk/content/enforced/201062-LR54398/itcSegment_troubleshooting_light.html
summary(se)
...
r <- rast("../data/Sciencedata/myDem_subset.tif") # TRY RASTER instead
r
plot(r)
?itcIMG()
...
```{r uclassify-aarhus}
a <- rast("../data/Sciencedata/Aarhus_1m.TIF")
plotRGB(a)
...
library(sf)
crs(a)
train <- read_sf("../data/Sciencedata/train_aarhus.geojson") %>% 
