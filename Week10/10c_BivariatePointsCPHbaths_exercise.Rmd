---
title: "Testing Bivariate spatial pattern in CPH Baths"
...
#baths_cph <- readRDS("../data/baths_cph.rds")  # baths by suburb centroids 
baths_osm <- readRDS("../data/baths_osm.rds") # OSM baths
```
...
## Watch the observed line swap sides
...
plot(kmult, . ~ r, main="Cross-K function")


```

If the observed line were below the envelope, there'd be repulsion or spatial separation between the two types. If it were above, there'd be significant clustering. In our case, the line stays within the envelope, which means:  No significant interaction — the spatial relationship is random, consistent with random labeling.

We tested whether "modern and historical WCs are located more closely together (or further apart) than we'd expect if we randomly shuffled the 'modern' vs 'historical' labels?" . We did not test whether each pattern is independently clustered — it's testing for cross-pattern interaction.
