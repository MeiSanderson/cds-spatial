...
```
This introductory script introduces a quadrat.test() to determine whether a spatial pattern is clustered, dispersed, or random. 
...
## Create some points within a vertical 1 x 2 rectangle 
...
# Task 2: Plotting areas with correct aspect ratio
...
Quadrat count tests are implemented using `quadrat.test()` in spatstat, which takes a planar point pattern, `ppp()` object and a number of subdivisions along x and y axis (`nx` and `ny`). "Planar point pattern" is jargon for a set of points in a region of a 2D plane
...
qt <- quadrat.test(ppxy, nx = 3, ny =3)
...
print(qt)
```
The p-value is likely over 0.05, which means there’s no evidence to reject CSR — the observed pattern is consistent with randomness. 
The "two-sided" tells you that the test was checking for both clustering and over-regularity, but found neither to be statistically significant.
...
# Create a disc of radius 10 if you have not done so earlier


# Generate clustered points from a Thomas process
set.seed(123)
?rThomas
p_cluster <- rThomas(kappa = 0.35, scale = 1, mu = 2, win = disc10)
plot(p_cluster)

# Run a quadrat test
qt <- quadrat.test(p_cluster, alternative = "clustered")
plot(qt)
print(qt)
...
# Run a quadrat test
qt_r <- quadrat.test(p_regular, alternative = "regular")
plot(qt_r)
print(qt_r)
...
# Test each pattern twice, once against regular and once against clustered hypothesis
qt1 <- quadrat.test(p1, alternative = "regular")
qt1 <- quadrat.test(p1, alternative = "clustered")
```
