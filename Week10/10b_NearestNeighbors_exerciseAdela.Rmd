---
title: "Point Patterns: Nearest Neighbor exercises"
...
G-force! `G`has lots of other function friends, including `F`, `H`,` J`, `K`, and `L`. Notice how `G` for the regular point pattern stays low at small distances, indicating very low probabilities of finding close pairs of points.
...
The K-function (often referred to as "Ripley's K") is defined as the expected number of points within a distance of a point of the process, scaled by the intensity. Like G, this can be computed theoretically for a uniform Poisson process and is `K(r) = pi * r ^ 2` - the area of a circle of that radius. Deviation from `pi * r ^ 2` can indicate clustering or point inhibition.
...
The plot method for estimates of `K` uses a formula system where a dot on the left of a formula refers to `K(r)`. So the default plot uses `. ~ r`. This shows the raw K function, which naturally grows like pi * r^² (a quadratic curve) — just like the area of a circle.  Why? Because under Complete Spatial Randomness (CSR), the number of expected neighbors grows with the area, and the area of a circle is πr².

You can compare the estimate of K to a Poisson process by plotting `. - pi * r ^ 2 ~ r`. If the data was generated by a Poisson process, then the line should be close to zero for all values of `r`, because under CSR, `K(r) - pi * r ^ 2 ~ r` equals 0, making the baseline horizontal.  Why use this? When you plot K(r) directly with `. ~ r`, the curve will naturally curve upward and make it harder to visually spot patterns, especially at larger r values. With horizontal line, deviations above or below zero are much easier to see.
...
* Use `envelope()` to simulate K for the random, regular, and clustered data.
  -   The first argument is the point pattern object, `p_poisson`.    
...
# Repeat for regular and clustered data

# Estimate the K-function for the regular points
K_regular <- Kest(p_regular, correction = "border")

# The default plot shows quadratic growth
plot(K_regular, . - pi * r^2 ~ r)
...
# Estimate the K-function for the clustered points
K_cluster <- Kest(p_cluster, correction = "border")

# The default plot shows quadratic growth
plot(K_cluster, . - pi * r^2 ~ r)
