...
# saveRDS(streets, "../data/Sciencedata/DGB_streets_osm.rds")
streets <- readRDS("../data/Sciencedata/DGB_streets_osm.rds")
streets <- streets %>% 
...
streets <- streets %>% select(osm_id, name, surface) # simply to work with less data at the moment
...
dem <- raster("../data/Sciencedata/DNK_msk_alt.grd") 
...
# choose the parameters you want. now max speed is 30 and min speed is 20
...
# just streets above, no we make a network (e.g., with connections; a vertex when streets cross)
...
tidygraph::with_graph(network, tidygraph::graph_component_count()) # disconnected sets of streets (incl. working paths)
```
...
round(node_count_largest / node_count_all * 100) # % of nodes within the biggest subdivision (?)
```
...
times <-valid_network %>%
  sfnetworks::st_network_cost(venue, BDG|>st_intersection(area), weights = "time") # cost network (cost = time); how many BDG can we get to

access <- BDG[which(times < units::set_units(6, "min")), ] # how many BDG (shelters) can we get to within 6 min biking
...
route_edges <- valid_network %>%
  ...
  st_as_sf() # convert into simple feature
