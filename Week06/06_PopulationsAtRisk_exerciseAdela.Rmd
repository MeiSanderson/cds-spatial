...
hh_no_cars <- readRDS("../data/hh_nocar.rds")
library(geodata)
munic <- gadm(country = "DNK", path = ".", level = 2)
munic <- readRDS("../data/gadm36_DNK_2_sp.rds")


# Fix names
hh_no_cars$municipality 

munic$NAME_2[31] <- "Aarhus"
munic$NAME_2[21] <- "Høje-Taastrup"
munic$NAME_2[60] <- "Vesthimmerlands"



no_cars_pct_mun <- munic %>% 
  st_as_sf() %>% 
  inner_join(hh_no_cars, by = c("NAME_2" = "municipality"))
```
...
all.equal(st_crs(no_cars_pct_mun), st_crs(cycling_isos))
all.equal(st_crs(no_cars_pct_mun), st_crs(driving_isos))

# Outside 20-min Cycling range
no_cars_pct_mun %>% 
  st_transform(crs = 4326) %>%  # ensure CRS compatibility with cycling isos
  filter(nc2021pct > 25) %>%  # filter 25%+ carless households
  filter(NAME_1 == "Midtjylland") %>%  # only Midtjylland
  st_union() %>%   # unioning the Midt carless household municipalities into a single multipoly
  st_difference(st_union(cycling_isos)) %>% 
  st_area()/1000000
  mapview()

# Outside 20-min Driving range
no_cars_pct_mun %>% 
  st_transform(crs = 4326) %>%  # ensure CRS compatibility with cycling isos
  filter(nc2021pct > 25) %>%  # filter 25%+ carless households
  filter(NAME_1 == "Midtjylland") %>%  # only Midtjylland
  st_union() %>%   # unioning the Midt carless household municipalities into a single multipoly
  st_difference(st_union(driving_isos)) %>% 
  #st_area()/1000000
  mapview()
```
